...
namespace App\Twig\Templating;

use App\Twig\Templating\Placeholder\CacheBusterAware;
use App\Twig\Templating\Placeholder\Container;
use App\Twig\Templating\Placeholder\ContainerService;
use App\Twig\Templating\Placeholder\Exception;
use App\Twig\Templating\Traits\WebLinksTrait;
...
class HeadLink extends CacheBusterAware
{
    ...
    protected function prepareEntries()
    {
        foreach ($this as &$item) {
            ...
            if (is_array($itemAttributes) && !empty($source)) {
                $this->handleWebLink( $source, $itemAttributes);
            }
        }
    }
    ...
    public function createDataAlternate(array $args)
    {
        ...
        if(empty($title)){
            $attributes = compact('rel', 'href', 'type', 'extras');
        }
